...
public class PublishFilterDirectoryScannerTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		File srcFile = BundleUtils.getFileLocation("serverMock/" + "eap5.0.mf.twiddle.jar");
...
//      WEB-INF/pages/common.html
		...
		MockModule mock = MockModuleUtil.createMockWebModule();
		mock.setMembers(members);
		this.module = mock;
	}
	...
	public void testIncludesSpecificFolder() throws CoreException {
		PublishFilterDirectoryScanner scanner = new PublishFilterDirectoryScanner(members);
		...
		ModuleDirectoryScannerPathFilter filt = new ModuleDirectoryScannerPathFilter(module, scanner);
		IModuleResource[] cleaned = filt.getFilteredMembers();
		assertEquals(27, countAllResources(members));
		int postTotal = countAllResources(cleaned);
		assertEquals(4, postTotal);
	}

	public void testExcludesDeepFolder() throws CoreException {
		PublishFilterDirectoryScanner scanner = new PublishFilterDirectoryScanner(members);
		...
		assertEquals(24, countAllResources(getCleanedMembers(scanner)));
	}
	
	public void testExcludesTrailingSlash() throws CoreException {
		PublishFilterDirectoryScanner scanner = new PublishFilterDirectoryScanner(members);
		scanner.setIncludes(new String[]{"**/*"});
		scanner.setExcludes(new String[]{"WEB-INF/pages"});
		scanner.scan();
		assertEquals(27, countAllResources(getCleanedMembers(scanner)));
	}
	public void testExcludesTrailingSlash2() throws CoreException {
		PublishFilterDirectoryScanner scanner = new PublishFilterDirectoryScanner(members);
		scanner.setIncludes(new String[]{"**/*"});
		scanner.setExcludes(new String[]{"WEB-INF/pages/"});
		scanner.scan();
	...
	public void testCommaSeparatedExcludes() throws CoreException {
		PublishFilterDirectoryScanner scanner = new PublishFilterDirectoryScanner(members);
		scanner.setIncludes(new String[]{"**/*"});
		scanner.setExcludes(new String[]{ "WEB-INF/pages/","WEB-INF/classes/foo/bar/"});
		scanner.scan();
		assertEquals(22, countAllResources(getCleanedMembers(scanner)));
	}

	public void testExcludesMorePowerful() throws CoreException {
		PublishFilterDirectoryScanner scanner = new PublishFilterDirectoryScanner(members);
		scanner.setIncludes(new String[]{"**/*","WEB-INF/classes/foo/bar/"});
	...
	private IModuleResource[] getCleanedMembers(PublishFilterDirectoryScanner scanner) throws CoreException {
		ModuleDirectoryScannerPathFilter filt = new ModuleDirectoryScannerPathFilter(module, scanner);
		IModuleResource[] cleaned = filt.getFilteredMembers();
		return cleaned;
	}
