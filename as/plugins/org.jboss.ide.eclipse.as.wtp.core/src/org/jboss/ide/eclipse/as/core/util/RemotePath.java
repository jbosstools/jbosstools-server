...
public class RemotePath implements IPath, Cloneable {
	...
	/** Target system's separator character*/
	private char targetSystemSeparator;
	...
	public static IPath fromPortableString(String pathString) {
		...
		if (firstMatch <= 0)
			return new RemotePath().initialize(null, pathString);
		...
		if (pathString.indexOf(DEVICE_SEPARATOR) == -1)
			return new RemotePath().initialize(devicePart, pathString);
		...
		return new RemotePath().initialize(devicePart, new String(chars, 0, writeOffset));
	}
	...
	public RemotePath(String fullPath) {
		this(fullPath, java.io.File.pathSeparatorChar);
	}
	 ...
	 *
	 * @param fullPath the string path
	 * @param targetSystemSeparator  the separator character used on the target system
	 * @see #isValidPath(String)
	 */
	public RemotePath(String fullPath, char targetSystemSeparator) {
		// keep ref on target system's separator
		this.targetSystemSeparator = targetSystemSeparator;
		
		String devicePart = null;
		if (isTargetSystemWindows()) {
	...
	public RemotePath(String device, String path) {
		this(device, path, java.io.File.separatorChar);
	}
	 ...
	 * @param device the device id
	 * @param path the string path
	 * @param separator the target system's separator character
	 * @see #isValidPath(String)
	 * @see #setDevice(String)
	 */
	public RemotePath(String device, String path, char targetSystemSeparator) {
		this.targetSystemSeparator= targetSystemSeparator;
		if (isTargetSystemWindows()) {
			//convert backslash to forward slash
	 ...
	 * Private constructor.
	 */
	private RemotePath(String device, String[] segments, int _separators, char targetSystemSeparator) {
		this.targetSystemSeparator = targetSystemSeparator;
	...
	public IPath addFileExtension(String extension) {
		...
		return new RemotePath(device, newSegments, separators, targetSystemSeparator);
	}
	...
	public IPath addTrailingSeparator() {
		...
		return new RemotePath(device, segments, separators | HAS_TRAILING, targetSystemSeparator);
	}
	...
	public IPath append(IPath tail) {
		...
		RemotePath result = new RemotePath(device, newSegments, (separators & (HAS_LEADING | IS_UNC)) | (tail.hasTrailingSeparator() ? HAS_TRAILING : 0), targetSystemSeparator);
	...
	public IPath append(String tail) {
		...
		if (tail.indexOf(SEPARATOR) == -1 && tail.indexOf("\\") == -1 && tail.indexOf(DEVICE_SEPARATOR) == -1) { //$NON-NLS-1$
			...
			return new RemotePath(device, newSegments, separators & ~HAS_TRAILING, targetSystemSeparator);
		}
		//go with easy implementation
		return append(new RemotePath(tail));
	}
	...
	public boolean isValidPath(String path) {
		RemotePath test = new RemotePath(path);
	...
	public IPath makeAbsolute() {
		...
		RemotePath result = new RemotePath(device, segments, separators | HAS_LEADING, targetSystemSeparator);
	...
	public IPath makeRelativeTo(IPath base) {
		...
		return new RemotePath(null, newSegments, separators & HAS_TRAILING, targetSystemSeparator);
	}
	...
	public IPath makeUNC(boolean toUNC) {
		...
		return new RemotePath(toUNC ? null : device, segments, newSeparators, targetSystemSeparator);
	}
	...
	public IPath removeFirstSegments(int count) {
		...
		if (count >= segments.length) {
			return new RemotePath(device, NO_SEGMENTS, 0, targetSystemSeparator);
		}
	...
	public IPath removeLastSegments(int count) {
		...
		if (count >= segments.length) {
			...
			return new RemotePath(device, NO_SEGMENTS, separators & (HAS_LEADING | IS_UNC), targetSystemSeparator);
		}
		Assert.isLegal(count > 0);
		int newSize = segments.length - count;
		String[] newSegments = new String[newSize];
	...
	public IPath removeTrailingSeparator() {
		...
		return new RemotePath(device, segments, separators & (HAS_LEADING | IS_UNC), targetSystemSeparator);
	}
	...
	public IPath setDevice(String value) {
		...
		return new RemotePath(value, segments, separators, targetSystemSeparator);
	}
	...
	public String toOSString() {
		...
		if (resultSize <= 0)
			return EMPTY_STRING;
		char FILE_SEPARATOR = targetSystemSeparator;
