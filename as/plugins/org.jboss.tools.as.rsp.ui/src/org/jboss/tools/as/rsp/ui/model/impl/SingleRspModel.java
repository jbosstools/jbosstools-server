...
/**
 * The model for a single RSP, including its type, client, jobs currently
 * executing, and state for each of the servers declared on the RSP (including
 * deployment state)
 */
public class SingleRspModel {
	...
	public void removeJob(JobHandle jobHandle) {
		for (JobProgress jp : new ArrayList<>(jobs)) {
			if (jp.getHandle().getId().equals(jobHandle.getId())
					&& jp.getHandle().getName().equals(jobHandle.getName())) {
				jobs.remove(jp);
			}
		}
	}
	...
	private ServerState findServerState(ServerHandle serverHandle) {
		for (ServerState ss : new ArrayList<>(serverState)) {
			...
			if (ssh.getId().equals(serverHandle.getId())
					&& ssh.getType().getId().equals(serverHandle.getType().getId()))
				return ss;
		}
		return null;
	}
