...
public abstract class DownloadRuntimeHyperlinkComposite extends Composite {
	
	protected Link downloadAndInstallButton;
	private IWizardHandle handle;
	private TaskModel tm;
	public DownloadRuntimeHyperlinkComposite(Composite parent, int style, IWizardHandle handle, TaskModel tm) {
		super(parent, style);
		this.handle = handle;
		this.tm = tm;
		createWidgets();
	...
	protected class DownloadAndInstallListener extends SelectionAdapter {
		...
		public void widgetSelectedWorkaround(SelectionEvent se) {
			IRuntimeType type = getRuntimeTypeFromTaskModel();
							...
							if( unzipLoc != null ) {
								new Job("Update wizard buttons") {
									protected IStatus run( IProgressMonitor monitor) {
										Display.getDefault().asyncExec(new Runnable() {
											public void run() {
												postDownloadRuntimeUpdateWizard(unzipLoc);
											}
										});
	...
	private void fireInitialWidgetUpdates() {
		new Job("Update Download Runtimes Hyperlink") {
			protected IStatus run(IProgressMonitor monitor) {
				final DownloadRuntime[] downloads = DownloadRuntimeServerUtil.getDownloadRuntimes(getRuntimeTypeFromTaskModel());
				Display.getDefault().asyncExec(new Runnable() { 
					public void run() {
						if( downloads != null && downloads.length > 0 ) {
							try {
								...
								setToolTipText(Messages.rwf_downloadTooltipEmpty);
						}
					}
				});
				return Status.OK_STATUS;
