 ...
 */
public class Wildfly9Manager {
	public static final int DEFAULT_REQUEST_TIMEOUT = 5000;
	private ModelControllerClient client;
	private ServerDeploymentManager manager;
	private IAS7ManagementDetails details;

	public Wildfly9Manager(IAS7ManagementDetails details) throws JBoss7ManangerException {
		try {
	...
	public JBoss7DeploymentState getDeploymentState(String name) throws JBoss7ManangerException {
		...
		Boolean enabled = Wildfly9ManagerUtil.getBooleanProperty(ENABLED, result);
		if (enabled == null) {
	...
	/*package*/ ModelNode execute(ModelNode node) throws JBoss7ManangerException {
		try {
			...
			if (!Wildfly9ManagerUtil.isSuccess(response)) {
				throw new JBoss7ManangerException(
